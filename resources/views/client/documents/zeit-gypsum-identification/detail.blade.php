        ...
        <div class="right-col">
            <nav style="--bs-breadcrumb-divider: '\e900';" aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item">
                        <a href="{{ get_link($slugs, [4, 16]) }}">
                            {{ __('message.identify-standar-plaster') }} </a>
                    </li>
                </ol>
            </nav>
            <h2 class="heading-1 title">{{ $identification->first()->title }}</h2>
        </div>
    </div>
</div>
<span class="lines">
...
<section class="related-news-sect">
    <div class="container">
        <h4 class="heading-1 text-uppercase text-blue">{{ __('message.identify-heading') }}</h4>
        <div class="news-list type-2">
            @forelse ($identifications as $key => $data)
                @if($data->id != $identification->first()->id)
                <div class="news-item">
                    <div class="inner">
                        <div class="thumb has-hover-img">
                            <img src="{{ asset('storage/assets/img/identification/' . $data->avatar) }}" alt="#">
                        </div>
                        <h2 class="heading-3 name"><a href="{{ get_link($slugs, [4, 16 , $data->slug]) }}">{{ $data->title }}</a>
                        </h2>
                        <div class="desc">
                            <p></p>
                        </div>
            ...
            <div class="content">
                ...
                <form action="#" class="form-subscription" novalidate>
                    <div class="form-group">
                        <input class="form-control" placeholder="{{ __('message.placeholder-input-detail') }}"
                            type="email" name="subscribtionEmail" id="subscribtionEmail" required>
