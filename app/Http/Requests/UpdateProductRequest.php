...
class UpdateProductRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'new_thickness.*' => ['required', 'numeric', 'min:0', 'max:10000000000'],
            'thickness_unit.*' => 'required',    
            'new_price.*' => ['required','numeric','min:0','max:10000000000'],
            'new_thickness_unit.*' => 'required'
        ];
        ...
        if ($this->input('category_id') != 13) {
            ...
            $rules['new_width.*'] = ['required', 'numeric', 'min:0', 'max:100000000'];
            $rules['new_height.*'] = ['required', 'numeric', 'min:0', 'max:100000000'];
        }
    ...
    public function messages()
    {
        return [
            ...
            'new_height.*' => [
                ...
                'numeric' => 'Chiều cao phải là số.',
                'min' => 'Chiều cao phải lớn hơn 0.',
                'max' => 'Chiều cao lớn vui lòng nhập bé hơn.',
            ],
            'new_thickness_unit.*' => [
                'required' => 'đơn vị không được để trống.',
            ]
